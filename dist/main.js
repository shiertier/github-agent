...
function loadUserConfig() {
    ...
    if (fs.existsSync(tomlPath)) {
        try {
            ...
            tomlConfig = {
                ...
                fallback_models: parsed.fallback_models,
    ...
    if (modelName)
        inputConfig.model_name = modelName;
    const maxTokens = core.getInput("max_tokens");
    if (maxTokens)
        inputConfig.max_tokens = parseInt(maxTokens, 10);
    const fallbackModels = core.getInput("fallback_models");
    if (fallbackModels)
        inputConfig.fallback_models = fallbackModels.split(",").map((s) => s.trim());
    const responseLang = core.getInput("response_language");
    if (responseLang)
        inputConfig.response_language = validateResponseLanguage(responseLang);
    const maxRounds = core.getInput("max_rounds");
    if (maxRounds)
        inputConfig.max_rounds = parseInt(maxRounds, 10);
    // 合并配置（后面的覆盖前面的）
    return {
        ...defaults,
        ...tomlConfig,
...
async function checkRoundLimit(maxRounds) {
    ...
    const lastResetIndex = comments
        ...
        .filter((i) => i >= 0)
        .pop();
    const relevantComments = lastResetIndex !== undefined
...
function normalizeCliName(binary) {
...
function installOpenCodeCli() {
    ...
    (0, child_process_1.execSync)("curl -fsSL https://raw.githubusercontent.com/opencode-ai/opencode/refs/heads/main/install | bash", { stdio: "inherit" });
...
function ensureCliAvailable(opencodeBin) {
    if (resolveExecutable(opencodeBin))
        return;
    const binaryName = normalizeCliName(opencodeBin);
    if (resolveExecutable(binaryName))
        return;
    if (binaryName === "codex") {
        installCodexCli();
    }
        ...
        throw new Error(`CLI binary not found: ${opencodeBin}. Set opencode_bin to "codex" or "opencode", or provide a valid path.`);
    }
...
function buildOpenCodeEnv(userConfig) {
    const env = {
        ...process.env,
...
// 运行 OpenCode/Codex
function runOpenCode(promptFile, userConfig, continueMode = false, overridePrompt) {
    ...
    if (cliName === "codex") {
        ...
        (0, child_process_1.execFileSync)(opencodeBin, args, {
            stdio: "inherit",
...
function verifyAndResume(config, maxRetries = 5) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        core.warning(`Missing files (attempt ${attempt}/${maxRetries}): ${missingFiles.join(", ")}`);
...
// 后处理：Issue Chatter
async function postProcessIssueChatter(round) {
    const token = core.getInput("github_token") || process.env.GITHUB_TOKEN;
    if (!token)
        throw new Error("Missing GITHUB_TOKEN");
    const octokit = github.getOctokit(token);
...
// 后处理：PR Reviewer
async function postProcessPRReviewer(round) {
    const token = core.getInput("github_token") || process.env.GITHUB_TOKEN;
    if (!token)
        throw new Error("Missing GITHUB_TOKEN");
    const octokit = github.getOctokit(token);
...
async function postProcessCoder(mode, round) {
    ...
    if (!issueNumber)
        return;
    const summaryFile = mode === "pr-coder"
...
// 后处理路由
async function postProcess(config, round) {
    switch (config.mode) {
        case "issue-chatter":
